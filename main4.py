...
def create_ba(node_list,N):
  ...
  node_list.append(Node(np.array[[1,MIN,10]]))
  node_list.append(Node(np.array[[0,MIN,10]]))
  ...
  for _ in range(N):
    ...
    node_list[select_node].connection == np.append(node_list[select_node].connection, [len(node_list),MIN,width], axis=0)
    # 新規ノードをnode_listに追加
    node_list.append(Node(np.array[[select_node,MIN,width]]))
...
def volatilize(node_list,V):
  ...
  for node in node_list:
    for j in range(len(node.connection)):
      ...
      # 最小フェロモン量を下回らないように
      if new_pheronone <= MIN:
        node.connection[j][1]=MIN
      else:
...
def update_pheromone(ant,node_list):
  ...
  for i in range(1,len(ant.route)):
    ...
    new_pheronone = before_node.connection[row][1] + (F * ant.minwidth)

    if new_pheronone >= MAX:
      before_node.connection[row][1] = MAX
    else:
      before_node.connection[row][1] = new_pheronone
...
def edge2node(edge_list,node_list):
  ...
  for edge in edge_list:
    ...
    before_node.connection = np.append(before_node.connection , np.array([[edge[1],MIN,width]]) , axis=0)
    after_node.connection = np.append(after_node.connection , np.array([[edge[0],MIN,width]]) , axis=0)
...
if __name__ == "__main__":
  ...
  #4×4のネットワークを生成
  node_list.append(Node(np.array([[1,MIN,100],[4,MIN,10]])))
  node_list.append(Node(np.array([[0,MIN,10],[5,MIN,100],[2,MIN,10]])))
  node_list.append(Node(np.array([[1,MIN,10],[6,MIN,10],[3,MIN,10]])))
  node_list.append(Node(np.array([[2,MIN,10],[7,MIN,10]])))
  node_list.append(Node(np.array([[0,MIN,10],[5,MIN,10],[8,MIN,10]])))
  node_list.append(Node(np.array([[1,MIN,10],[4,MIN,10],[6,MIN,100],[9,MIN,10]])))
  node_list.append(Node(np.array([[2,MIN,10],[5,MIN,10],[7,MIN,10],[10,MIN,100]])))
  node_list.append(Node(np.array([[3,MIN,10],[6,MIN,10],[11,MIN,10]])))
  node_list.append(Node(np.array([[4,MIN,10],[9,MIN,10],[12,MIN,10]])))
  node_list.append(Node(np.array([[5,MIN,10],[8,MIN,10],[10,MIN,10],[13,MIN,10]])))
  node_list.append(Node(np.array([[6,MIN,10],[9,MIN,10],[11,MIN,100],[14,MIN,10]])))
  node_list.append(Node(np.array([[7,MIN,10],[10,MIN,10],[15,MIN,100]])))
  node_list.append(Node(np.array([[8,MIN,10],[13,MIN,10]])))
  node_list.append(Node(np.array([[9,MIN,10],[12,MIN,10],[14,MIN,10]])))
  node_list.append(Node(np.array([[10,MIN,10],[13,MIN,10],[15,MIN,10]])))
  node_list.append(Node(np.array([[11,MIN,10],[14,MIN,10]])))
