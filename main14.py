...
V = 0.9 # フェロモン揮発量
M = 100 # フェロモン最小値
TTL = 25 # 蟻のTime to Live 出発ノード含めてTTLの数のノードを移動できる
W = 1000 # 帯域幅初期値
BETA = 0.5 # 経路選択の際のヒューリスティック値に対する重み(累乗)
...
if __name__ == "__main__":
  ...
  for i in range(5):
    ...
    for g in range(GENERATION):
      multi_simulation_log[g].append(interest_log[g][0])

  with open('./sample.csv', 'w') as f:
    writer = csv.writer(f)
    for m in multi_simulation_log:
      writer.writerow(m)
