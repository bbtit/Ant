...
def interest_next_node(interest_list:list[Interest] ,node_list:list[Node] ,interest_log:list[int], gen:int) -> None:
  # interestの次のノードを決定
  # 繰り返し中にリストから削除を行うためreversed
  for interest in reversed(interest_list):
    ...
    # 候補先がある場合
      ...
      # 次のノード番号がconnectの何番目か探索
      num = connection.index(next_node)
...
def show_node_info(node_list:list[Node]) -> None:
  for node in node_list:
    print(str(node.connection) + str(node.pheromone) + str(node.width))
...
if __name__ == "__main__":
  ...
  for _ in range(100):
    ...
    for gen in range(GENERATION):
      ...
      # Interestによる評価フェーズ
      # Interestを配置
      interest_list.extend( [ Interest(START_NODE,GOAL_NODE,[START_NODE],W) ] * INTEREST_NUM)
      # Interestの移動
      for _ in range(TTL):
