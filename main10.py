...
V = 0.9 # フェロモン揮発量
M = 10 # フェロモン最小値
TTL = 10 # 蟻のTime to Live 出発ノード含めてTTLの数のノードを移動できる
W = 1000 # 帯域幅初期値
ANT_NUM = 40 # 一回で放つAntの数
...
if __name__ == "__main__":
  ...
  for i in range(100):
    ...
    for gen in range(100):
      ...
      # Antの移動
      for _ in range(TTL):
        print("ant_list length : " + str(len(ant_list)))
    ...
    if (len(interest_log[-1]) ==  0):
      multi_simulation_log.append(0)
    else:
